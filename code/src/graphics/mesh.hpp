...
struct MeshVertexFormat {
    ...
    static inline const GLuint ARBITRARY_ATTRIBUTE_2_NAME = 14;
...
class Mesh {
    ...
    Mesh(const std::vector<GLfloat> &verts, const std::vector<GLuint> &indies, const MeshCreateParams& params, bool fromText = false, std::optional<std::vector<Bone>> bones = std::nullopt, std::optional<std::vector<Animation>> anims = std::nullopt, const unsigned int rootBoneIndex = 0);
