...
struct VertexAttribute {
    ...
    // the size, in bytes, of each vertex attribute. (position, being 3 floats, would = 3 * sizeof(GLfloat))
    unsigned int size;
...
struct MeshVertexFormat {
    union { // this cursed union lets us either refer to vertex attributes by name, or iterate through them using the member vertexAttributes[]
        struct {
            ...
            std::optional<VertexAttribute> modelMatrix; // one 4x4 model matrix per thing being drawn, multiplying vertex positions by this puts them in the right position via rotating, scaling, and translating
            std::optional<VertexAttribute> normalMatrix; // normal matrix is like model matrix, but is 3x3 and for normals since it would be bad if a normal got scaled/translated  
        ...
        std::optional<VertexAttribute> vertexAttributes[8];
    };
...
class Mesh {
    ...
    // Takes a VAO and sets its vertex attributes using VertexAttribPointer().
    // The VAO must ALREADY BE BOUND.
    void SetVaoVertexAttributes(GLuint& vaoId) const;
    ...
    void HandleAttribute(GLuint& vaoId, const std::optional<VertexAttribute>& attribute, const GLuint attributeName) const;
